...
class ConfigurationTest {
    ...
    private EnableNatsStreamingServer annotation;
    private AnnotationConfigApplicationContext ctx;
    private MergedContextConfiguration mCtx;
    private EnableNatsStreamingServerContextCustomizer contextCustomizer;
    ...
    void setUp() {
        ...
        annotation = mock(EnableNatsStreamingServer.class);
        when(annotation.port()).thenReturn(4222);
        when(annotation.timeoutMs()).thenReturn(512L);
        contextCustomizer = new EnableNatsStreamingServerContextCustomizer(annotation);
        final MutablePropertySources propertySources = ctx.getEnvironment().getPropertySources();
        final Map<String, Object> map = new HashMap<>();
        map.put("nats.streaming.server.nats_log_name", "NatsTest");
        propertySources.addFirst(new MapPropertySource("nats.streaming.server.nats_log_name", map));
    }
    ...
    void natsServer_configureForCovWithWrongPort_shouldFailToStart() {
        ...
        assertThrows(NatsStreamingStartException.class, () -> contextCustomizer.customizeContext(ctx, mCtx));
    }
}
